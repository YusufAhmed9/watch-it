...
//        }
    ...
    public void redirectToSubscriptionPage(ActionEvent actionEvent) {
        try {
            String css = getClass().getResource("/WatChill/style/Main.css").toExternalForm();
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/WatChill/Subscription/subscription.fxml"));
            root = loader.load();
            stage = (Stage) trendingSeriesContainer.getScene().getWindow();
            scene = new Scene(root);
            scene.getStylesheets().add(css);
    ...
    public void handleSearch(KeyEvent keyEvent) {
        ...
        if (searchType.equals("Series")) {
            ArrayList<Series> searchResults = Series.searchSeriesByTitle(searchQuery);
            ...
            ArrayList<Movie> searchResults = Movie.searchMoviesByName(searchQuery);
            for (int i = 0; i < searchResults.size(); i++) {
            ...
            ArrayList<Cast> searchResults = Cast.searchCastsByName(searchQuery);
            for (int i = 0; i < searchResults.size(); i++) {
                Cast cast = searchResults.get(i);
                try {
                    FXMLLoader loader = new FXMLLoader(getClass().getResource("/WatChill/Search/searchResult.fxml"));
                    HBox searchResult = loader.load();
            ...
            ArrayList<Director> searchResults = Director.searchDirectorsByName(searchQuery);
            for (int i = 0; i < searchResults.size(); i++) {
                Director director = searchResults.get(i);
                try {
                    FXMLLoader loader = new FXMLLoader(getClass().getResource("/WatChill/Search/searchResult.fxml"));
                    HBox searchResult = loader.load();
